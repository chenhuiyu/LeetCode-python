...
class Solution:
    def maxPathSum(self, root: Optional[TreeNode]) -> int:
        def loopInfo(root):
            ...
            maxPathSumFromHeadLeft, maxPathSumLeft = loopInfo(root.left) if root.left is not None else 0, float("-inf")
            maxPathSumFromHeadRight, maxPathSumRight = loopInfo(root.right) if root.right is not None else 0, float("-inf")
